...
class TeamModel
{
	...
	public function getAllTeam(){
		if($this->db) {
			$st = $this->db->prepare("SELECT team_id, team_name, team_short_name, league_name, team_city, team_stadium, team_avatar, team_fans_num FROM team,league WHERE team.league_id=league.league_id AND league.league_visible=1 ORDER BY team_id DESC");
			$st->execute();
	...
	public function getTeamList($nStart, $nTeamPerPage){
		...
		if($this->db) {
			$st = $this->db->prepare("SELECT * FROM team ORDER BY team_id DESC LIMIT :nStart, " . $nTeamPerPage);
